...
namespace Render{
	...
	:IRenderingStrategy()
	, m_quad(1, 1, 640, 480)
	, m_quad2(lpv_size, lpv_size*lpv_size, lpv_size, lpv_size*lpv_size){
	}
...
//			pRenderPass->SetFrameBuffer(GetRenderPass(RENDER_PASS_ENUM::LIGHT_INJECT)->GetFrameBuffer());
	...
	void DeferredRenderingStrategy::Render(const Camera& camera, std::function<void()> fRenderModels) {
		...
		FrameBuffer* pGPassFrameBuffer = pRenderPass0->GetFrameBuffer(0);
...
#define SHADER (*GetCurrentRenderPass())
		...
		RenderBegin(RENDER_PASS_ENUM::RSM, 0, true);
		...
		RenderBegin(RENDER_PASS_ENUM::DOWNSAMPLE_GBUFFER, 0, true);
			SHADER.BindTexture(SHADER_UNIFORM_ENUM::TEX0, pGPassFrameBuffer->m_uTextures[2]);
...
//		RenderEnd();
		...
		RenderBegin(RENDER_PASS_ENUM::POST_LPV, 0, true);
			SHADER.BindTexture(SHADER_UNIFORM_ENUM::LIGHT, pGPassFrameBuffer->m_uTextures[0]);
			SHADER.BindTexture(SHADER_UNIFORM_ENUM::GBUFFER_POS, pGPassFrameBuffer->m_uTextures[2]);
			SHADER.BindTexture(SHADER_UNIFORM_ENUM::GBUFFER_NORMAL, pGPassFrameBuffer->m_uTextures[1]);
...
//			SHADER.BindTextures(SHADER_UNIFORM_ENUM::LIGHT_VOLUME, GetRenderPass(RENDER_PASS_ENUM::LIGHT_INJECT)->GetFrameBuffer()->m_uTextures, 3);
...
//		vecTextures.push_back(pGPassFrameBuffer->m_uTextures[1]); //normal
		...
		vecTextures.push_back(GetRenderPass(RENDER_PASS_ENUM::DOWNSAMPLE_GBUFFER)->GetFrameBuffer(0)->m_uTextures[0]); //pos
		for(int i=0; i<3; ++i){
			vecTextures.push_back(GetRenderPass(RENDER_PASS_ENUM::LIGHT_INJECT)->GetFrameBuffer(0)->m_uTextures[i]);
...
//		vecTextures.push_back(GetRenderPass(RENDER_PASS_ENUM::QUAD_TEST)->GetFrameBuffer()->m_uTextures[0]); //depth
		vecTextures.push_back(pGPassFrameBuffer->m_uTextures[3]);


		RenderToScreen(viewport, RENDER_PASS_ENUM::RENDER_TO_SCREEN, 2, vecTextures);
